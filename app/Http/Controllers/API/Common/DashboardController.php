...
class DashboardController extends Controller
{
    ...
    public function dashboard()
    {
        /*
        $alldprs = DprMap::get();
        foreach($alldprs as $dpr)
        {
            $itemdesc = ItemDescriptionMaster::where('title', $dpr->item_desc)->first();
            $dpr->item_Desc_id = $itemdesc->id;
            $dpr->save();
        }

        $allimports = DprImport::get();
        foreach($allimports as $dpr)
        {
            $itemdesc = ItemDescriptionMaster::where('title', $dpr->item_desc)->first();
            $dpr->item_Desc_id = $itemdesc->id;
            $dpr->save();
        ...
        try {
            ...
            if(!empty(auth()->user()->vendor_id)){
                ...
                $data['totalVendor'] = Vendor::join('dpr_configs','vendors.id','dpr_configs.vendor_id')
                 ->where('dpr_configs.vendor_id',auth()->user()->vendor_id)
                 ->count();
                $data['TotalWorkPackage'] = ItemDescriptionMaster::join('dpr_configs','item_description_masters.title','dpr_configs.item_desc')
                 ->where('dpr_configs.vendor_id',auth()->user()->vendor_id)
                 ->groupBy('dpr_configs.item_desc')
                 ->count();
            ...
            if(!empty($request->month))
            {
                ...
                for ($d=1; $d<=$last_date; $d++) {
                    ...
                    if(!empty(auth()->user()->project_ids))
                    {
                        ...
                        $dprUploads = $dprUploads->whereIn('dpr_configs.project_id',$project_ids);
                    }
    ...
    public function itemDescList(Request $request)
    {
        try {
            ...
            $query = ItemDescriptionMaster::orderby('orderno','ASC');
